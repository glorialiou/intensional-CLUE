...
--Initial set of possible worlds
worlds :: [World]
...
--Generates correct world from int input
...
-- *** CLUE: Added iKiller, iWeapon, and iCrimescene
iGirl, iPrincess, iPerson, iKiller, iWeapon, iCrimescene
       :: World -> Entity -> Bool
iGirl     W1 = girl
iGirl     W2 = girl'
iGirl     W3 = girl' 
...
-- *** CLUE: Added iIs
iCatch, iIs :: World -> Entity -> Entity -> Bool
iCatch W1 = \ x y -> False
iCatch W2 = \ x y -> False
...
-- *** CLUE: Added profs, classes, and locations to iNP
...
iDET No p q = \ i -> not (any (\x -> q (\j -> x) i) 
      (filter (\x -> p (\j -> x) i) entities))
iDET The p q = \ i -> singleton (filter (\x -> p (\j -> x) i) entities)
     && (\x -> q (\j ->x) i) (head (filter (\x -> p (\j -> x) i) entities))
       where
          singleton [x] = True
          singleton _ = False

-- *** CLUE: Added iTV to interpret transitive verbs 
iTV Is = \ x y i -> iIs i (x i) (y i)

-- *** CLUE: Added iVP (VP1 tv np) to VPs including a tv and an object
...
-- *** CLUE: Added interpretations of Killer, Weapon, Crimescene
